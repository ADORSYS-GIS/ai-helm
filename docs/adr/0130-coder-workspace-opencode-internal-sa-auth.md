...
- The SA is named **`coder-<sub>.<plan>.<workspaceId>`** (owner's identity, billing plan, and the workspace UUID in the *name*), set by the template at provisioning from the decoded login JWT. The `<workspaceId>` suffix makes the SA name **unique per workspace**: without it the name is keyed to the *owner* (`coder-<sub>.<plan>`), so a second workspace of the same user (e.g. a task-provisioned workspace while another is running) collides with `serviceaccounts "... already exists"` and terraform apply fails — verified live. UUIDs never contain `.`, so the parse below is unaffected.
- **Authorino** validates the SA token via `kubernetesTokenReview`, then derives both values by **pure CEL** from the SA name: take the last `:`-segment (the SA name), split on `.` (the plan delimiter), and strip the fixed `coder-` prefix for the `sub`. The optional third segment (the workspace UUID) is identity-neutral — `sub` = `[0]`, `plan` = `[1]` regardless of whether the suffix is present. No SA-label read, no Kubernetes-API metadata call, and the parse is **robust to any `sub` length** (the `.` never appears in a Keycloak `sub`, a plan, or a UUID).
...
- **Plan encoded in the SA name** — chosen (this ADR): both `sub` and plan derive from the name via pure CEL, delimited by `.` (robust to any `sub` length; the workspace UUID adds a third, identity-neutral segment for name uniqueness). Trade-off: plan changes need a workspace rebuild.
