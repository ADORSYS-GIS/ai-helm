...
| [0041](./0041-firecrawl-protocol-version-rewrite-nginx-engine.md) | **nginx proxy engine** for firecrawl (`charts/mcp` `proxy.engine: nginx` + `rewriteResponseProtocolVersion`): firecrawl (per spec) negotiates the client's `protocolVersion 2025-11-25` DOWN to `2025-06-18`, but the AIEG v0.7.0 mcpproxy demands an exact echo → 500 `failed to create MCP session to any backend`. Caddy can't rewrite a response body; nginx `sub_filter` rewrites the echoed version. INTERIM (pinned to `2025-11-25`) pending an AIEG fix; reverted the wrong `statelessUpstream` synthesis | Accepted | 2026-06-11 | extends [0040](./0040-external-mcps-via-caddy-normalizing-proxy.md) |
