...
| **Z-Image-Turbo is federated without a load gate, at a placeholder price** (ADR-0100) | No measured images/hour, VRAM or latency on this hardware; `$0.005/image` has no ADR-0096 cost basis | Accepted deliberately — it is a MIGRATION of a model users already had, not a new offering. Load gate + price re-derivation tracked in `inference-ops` |
