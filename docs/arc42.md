...
| `inference` → `inference-server` (catalog in `ai-helm-values`, ADR-0129) | Orchestrator ApplicationSet → one Application per self-hosted model on the Hetzner GPU fleet (`inference` ns, `home-remote`). THREE engine profiles (llama.cpp / vLLM+LMCache / **LocalAI**, image generation) expand a ~15-line catalog entry into the workload. **One GPU node (`hetzner-k8s-gpu-1`) since 2026-09-15 (ADR-0138, down from two) ⇒ one concurrently-served model.** Cluster-local: no Ingress, cert, API key or proxy sidecar | Orchestrator + leaves (ADR-0094/0095/0100/0102) |
...
| 0129 | **GPU fleet catalog moves to `ai-helm-values`** — the last model-shaped file left in the chart repo. Same shape as 0126; the prune hazard is worse (an empty catalog deletes each model's PVC of downloaded weights, not just its routes), so the requireCatalog guard is load-bearing. Both catalogs now sit in one repo, making the cross-catalog check local again |
| 0135 | **opencode logs in to our own IdP** — the well-known's `oauth2` block repoints from the Keycloak realm to `authz-idp` (`https://auth.ai.camer.digital`) with a new `opencode-cli` public client (device_code + refresh_token, PKCE) registered in `ai-helm-values`. Deliberately NOT a plugin swap: `authz-idp`'s device-code grant already mints a project-scoped token whose `iss` is what the gateway's `lightbridge-apikey` identity is pinned to, so no exchange is needed — and `@vymalo/opencode-lightbridge` always exchanges (a self-issued token can't be its own `subject_token` against the single Keycloak `jwks_url`), while `@vymalo/opencode-oauth2` can't be removed because it registers the provider and discovers models. Pins the contract that `x-account-id` is the ACTING lightbridge account id: byte-identical today by lightbridge-authz ADR-0025 grandfathering, but the Stage 5 audit this repo owes |
...
| **GPU fleet storage has no node redundancy** (ADR-0138, 2026-09-15) | `hetzner-k8s-gpu-2` was decommissioned, leaving `aii-longhorn` a one-node pool; `persistence.defaultClassReplicaCount` dropped 2→1 (a 2-replica volume can't be satisfied on one node). The GPU node's disk is the only copy of cached model weights | Accepted — the volumes hold re-fetchable weights (HF pulls), not primary data; a node loss forces a re-seed, not data loss. Revisit if a second GPU node returns |
