...
| `model-serving` → `model-server` | Orchestrator ApplicationSet → one Application per self-hosted model on the Hetzner GPU fleet (`inference` ns, `home-remote`). THREE engine profiles (llama.cpp / vLLM+LMCache / **zimage**, image generation) expand a ~15-line catalog entry into the workload. Cluster-local: no Ingress, cert, API key or proxy sidecar | Orchestrator + leaves (ADR-0094/0095/0100) |
| `model-serving-*` (qwen3-5, qwen3-4b, deepseek-r1-1-5b, qwen25-3b-awq, qwen3-8b, ministral-3b, qwen2-vl-2b, zimage-turbo) | ⚠️ **LEGACY, ALL DISABLED since 2026-07-27 (ADR-0100)** — per-model charts targeting the *other* cluster (`admin@homeos`) over a public edge; `homeCluster: true`. `zimage-turbo` was the last live one and moved to the fleet, so the generation is now a rollback surface only | Hybrid bjw (ADR-0022/0029/0030/0032) |
| `images/z-image-turbo-server/` (not a chart) | First-party Rust + Candle image-generation server behind the `zimage` engine profile; OpenAI `/v1/images/generations`. ⚠️ Built and pushed **out-of-band** — no CI builds it, so the tag must exist before a catalog entry referencing it merges | Container source (ADR-0100) |
...
| **No CI builds our first-party container images** (ADR-0100) | `lakefs-proxy` and `z-image-turbo-server` are built and pushed by hand; a chart referencing a tag that was never pushed sits in `ImagePullBackOff`, and the running image cannot be traced to a commit | Build-first ordering discipline, documented per image. A path-filtered build workflow is the fix and is not written |
| **The `zimage` engine publishes no `/metrics`** (ADR-0100) | `up{namespace="inference"}` is blind to image models; no engine dashboard, no tokens/s or queue-depth equivalent | `ms-model-unavailable` alert reads `kube_deployment_status_replicas_available` from kube-state-metrics (engine-independent); GPU-level DCGM telemetry unaffected |
| **The ADR-0097 CORS pin is unenforceable on `zimage`** (ADR-0100) | The server hardcodes `allow_any_origin()`; fleet policy silently does not apply to this engine | Declared `corsConfigurable: false` rather than hidden; contained by the CiliumNetworkPolicy (no browser can reach the pod) and by there being no credential an origin protects. Fixable in our own source |
| **Image generation is unavailable to users until its load gate passes** (ADR-0101, amending ADR-0100) | Z-Image-Turbo was briefly federated unmeasured on a "migration ≠ new offering" argument; two of three size estimates then failed on the first sync while users could still select it | `z-image-turbo-local` `enabled: false`; the serving entry stays enabled so it deploys and can be measured. **The load gate has no migration exception** — "it worked on the old hardware" is not evidence about the new hardware |
