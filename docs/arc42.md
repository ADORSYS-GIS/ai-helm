...
| 0100 | Image generation joins the GPU fleet as a THIRD engine profile (`zimage`), backed by a first-party Rust/Candle server (`images/z-image-turbo-server/`); Z-Image-Turbo moves off the home GPU's public edge and OpenMythos-27B is retired to free the card. Engine facts (`apiKey.mode`, `metrics`, `devShm`) become profile DATA rather than per-engine template branches. ⚠️ **Engine choice superseded by ADR-0102** — the first-party server was replaced by LocalAI one day later |
...
| **Rust images scan empty under Trivy without `cargo-auditable`** | A green scan covers only the base OS — zero crates inspected | `lakefs-proxy` builds with `cargo-auditable` (Trivy then reports a `rustbinary` target). The `z-image-turbo-server` first-party image was **deleted (ADR-0102)**, so this gap is now scoped to future Rust images |
...
| **Image serving depends on a moving gallery reference** (ADR-0102) | LocalAI resolves `MODELS` against its own gallery, so — unlike a seed Job pinned to a commit SHA — the bytes served can change under us | Accepted trade for not owning a server; the *image* tag is pinned and the load gate is the check. Fallback is a model config file we own, mounted into `MODELS_PATH` |
