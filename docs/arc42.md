...
| ~~**Image serving depends on a moving gallery reference**~~ **CLOSED by ADR-0105** | Was: LocalAI resolved `MODELS` against its own gallery, so the bytes served could change under us | We now define the model ourselves with a `sha256` per file in `download_files`, and pin the backend gallery + backend image tag to the server's own release under a keyless-cosign policy. ⚠️ Residual: the cosign identity regex and the `download_files` fetch are **unexercised** — the volume was already populated. The next boot (ADR-0106 pruned it) is the first real test |
...
| **Image generation is federated only because its load gate passed** (ADR-0101, amending ADR-0100) | Z-Image-Turbo was once federated unmeasured on a "migration ≠ new offering" argument; two of three size estimates then failed on the first sync while users could still select it | Gate passed 2026-07-28 on the tuned config — 32.4 s / 1024×1024, 7985 MiB, image generated and **looked at** — so `z-image-turbo-local` is `enabled: true` (ADR-0106). **The load gate has no migration exception**: "it worked on the old hardware" is not evidence about the new hardware, and neither is a green ArgoCD tree |
