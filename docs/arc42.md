...
| 0048 | Global `@vymalo/opencode-browser` plugin (page+control, no `debug`); **removes the `chrome-devtools` MCP**; promotes `frontend` to a LEAN default primary (`default_agent`) that delegates to model-less subagents (`@browser`, `@design`) inheriting its multimodal model; existing roles keep cost-lean pins; one source only (don't also connect the MCP form). Corrects ADR-0044's token model — opencode DOES scope the *injected* tool set per agent (`request.ts resolveTools`→`Permission.disabled`: string-form `*`-deny drops a tool from `tools`/`activeTools`, agent `allow` wins), so the lean primary never carries the 27 `browser_*` schemas |
