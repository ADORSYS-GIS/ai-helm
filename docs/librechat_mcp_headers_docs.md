...
Because we are in a production environment, directly port-forwarding to Envoy's admin interface is neither feasible nor secure. Instead, tracing should be performed by declaratively modifying the logging levels and consulting your centralized logging stack.
...
Apply this configuration via ArgoCD or your standard deployment pipeline.

### Analyzing the Envoy Trace

Once the configuration is synced, monitor the Envoy proxy logs either directly using `kubectl logs` or your centralized logging dashboard (e.g., Kibana, Datadog, or Grafana Loki).
...
```text
[2026-04-30 14:41:44.622][11][debug][ext_proc] [source/extensions/filters/http/ext_proc/processor_state.cc:67]
[2026-04-30 14:41:44.622][11][debug][router] [source/common/router/router.cc:830] cluster 'httproute/converse/...' router decoding headers:
':authority', 'services.api.ai.camer.digital'
':path', '/v1/chat/completions'
':method', 'POST'
...
'authorization', 'Bearer sk-converse-...'
'x-account-id', 'LIBRECHAT'
'x-project-id', 'sso:a897be3e-06ed-4f6b-b707-a507698da6e3'
'x-user-email', 'mfrancksorel@gmail.com'
'x-user-id', '69a817ebb7134749a237d5ea'
'x-user-role', 'ADMIN'
```
...
* **`x-user-email`**: Safely extracted the logged in user `mfrancksorel@gmail.com` matching the dynamic placeholder.
