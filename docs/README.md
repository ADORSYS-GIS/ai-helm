...
| [`librechat-chain-of-agents.md`](./librechat-chain-of-agents.md) | **Chain-of-Agent use cases** (exploration for #414): five sequential agent chains over the GitHub MCP — UC1 sprint→GitHub ticket factory, UC2 doc-drift remediation, UC3 ticket grooming, UC4 PR-review (the #407 context arms as a pluggable stage), UC5 external-team "envoy". Per-chain inputs/agents/instructions + cross-cutting findings (reusable sub-agents, adversarial-reviewer pattern, chains compose into pipelines, linear-chain limits, write-safety) |
