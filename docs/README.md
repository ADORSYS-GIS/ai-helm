...
| [`2026-07-31-dns01-route53-delegation-audit.md`](migrations/2026-07-31-dns01-route53-delegation-audit.md) | ⚠️ OPEN, for @stephane-segning — DNS-01 wildcards cannot be issued under `ai.camer.digital`: it is NS-delegated to Route53 while `cert-cloudflare` only serves `camer.digital`, so challenges stall on "not yet propagated" forever. `traefik/ai-certificate` stuck 9d (latent — nothing consumes it). Root cause, evidence, three remediation options |
