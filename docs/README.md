...
| [`chat-observability.md`](./chat-observability.md) | **Phoenix-style chat content, from the AI Gateway's already-live OpenInference traces** (ADR-0077 + ADR-0078): the ext-proc has been sending full-content spans to Tempo all along (`llm.input_messages.*`/`output_messages.*`), true in Phoenix's era too; the `chat-overview` (no input, embeddings excluded, live trace feed) and `chats-by-user` (Keycloak-sourced `$user` picker, ADR-0063; per-user trace feed + a per-request Loki metadata log) dashboards; the `controller.spanRequestHeaderAttributes` span-attribution that makes per-user CONTENT possible (ADR-0078, reversing 0077 — verified against a live span that content has no identity + no join key to the user-bearing Loki logs), the SQL-injection-avoidance note, and why single-chat drill-down isn't built (no conversation id forwarded yet) |
