...
| [`sonarqube.md`](./sonarqube.md) | **SonarQube Community Build** (ADR-0065) deploy + runbook: App-of-Apps (orchestrator + CNPG db + secrets), the community-branch-plugin version-lockstep contract (excluded from image-updater), the custom GitHub App PR-decoration binding, Keycloak SAML SSO, the node-sysctl prereq, and the out-of-band / values-repo-first checklist |
