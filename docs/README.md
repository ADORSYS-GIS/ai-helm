...
| File | What it covers |
|---|---|
| [`self-hosted-model-serving.md`](patterns/self-hosted-model-serving.md) | **The GitOps pattern for serving models on the Hetzner GPU fleet** (ADR-0094/0095): the `model-serving` orchestrator + generic `model-server` leaf, THREE engine profiles (llama.cpp / vLLM+LMCache / **zimage**, the first-party image-generation server — ADR-0100), GPU-as-a-resource-request scheduling, weight seeding, and cluster-local federation — no Ingress, cert, API key or proxy sidecar. Also documents the **legacy** per-model charts from `admin@homeos`, now all disabled. ⚠️ The *inference* knowledge (VRAM budgeting, quantization, engine choice, benchmarks, runbooks) lives in the team's **`inference-ops`** repo, not here. Legacy per-model papers: [`models/`](./models/) |
