...
| [`chat-observability.md`](./chat-observability.md) | **Phoenix-style chat content, from the AI Gateway's already-live OpenInference traces** (ADR-0077): the ext-proc has been sending full-content spans to Tempo all along (`llm.input_messages.*`/`output_messages.*`), true in Phoenix's era too; the `chat-overview` (no input, embeddings excluded, live trace feed) and `chats-by-user` (Keycloak-sourced `$user` picker, ADR-0063, Loki aggregates only) dashboards; why a `controller.spanRequestHeaderAttributes` per-user span-attribution PR was built then closed unmerged (live-verified unnecessary — content never depended on it), the SQL-injection-avoidance note, and why single-chat drill-down isn't built (no conversation id forwarded yet) |
