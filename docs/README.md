...
| File | What it covers |
|---|---|
| [`self-hosted-model-serving.md`](patterns/self-hosted-model-serving.md) | **The GitOps pattern for serving models on the Hetzner GPU fleet** (ADR-0094/0095): the `model-serving` orchestrator + generic `model-server` leaf, engine profiles (llama.cpp / vLLM+LMCache), GPU-as-a-resource-request scheduling, weight seeding, and cluster-local federation — no Ingress, cert, API key or proxy sidecar. Also documents the **legacy** per-model charts still serving from `admin@homeos`. ⚠️ The *inference* knowledge (VRAM budgeting, quantization, engine choice, benchmarks, runbooks) lives in the team's **`inference-ops`** repo, not here. Legacy per-model papers: [`models/`](./models/) |
