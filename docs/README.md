...
| [`continuous-delivery.md`](./continuous-delivery.md) | **The cutover runbook (ADR-0055)** — OCI-published charts + argocd-image-updater write-back to the private `ai-helm-values` repo: the new model, out-of-band prerequisites (values repo, the TWO creds, public GHCR package), phased cutover order, the per-app onboarding recipe, rollback, gotchas. **Supersedes** the tag-based `releasing.md` ↓ |
| [`releasing.md`](./releasing.md) | **(Superseded by [`continuous-delivery.md`](./continuous-delivery.md) / ADR-0055; applies only until the cutover completes.)** How to cut a tag-based release via `tools/release.sh` (bump self-ref `targetRevision`s → tag the commit → push tag first → repoint root `ai-apps-v2`); flags, safety, the manual root-repoint, rollback |
