...
| File | What it covers |
|---|---|
| [`coder-declarative-templates.md`](patterns/coder-declarative-templates.md) | Declarative Coder template management via the `coder/coderd` Terraform provider (GitOps; retire `coder templates push`) — decision + implementation pattern (ADR-0124) |
...
| [`self-hosted-code-interpreter.md`](patterns/self-hosted-code-interpreter.md) | **Self-hosted LibreChat Code Interpreter** (ADR-0122): vendored `clickhouse/code-interpreter`, the secret-generation checklist, install verification, and known limitations (NsJail sandbox mode, RWO-only storage, shared S3 bucket) |
...
| [`ratelimit-quota-observability.md`](patterns/ratelimit-quota-observability.md) | **Rate-limit quota from the limiter's LIVE Redis counters** (ADR-0070): the shared-budget key shape (rule index → plan, `<window>` epoch + calendar `billing_period` — ADR-0084/0111), the exporter→Mimir leaderboard + the `redis-datasource` tmscan census (`tlsAuth:true`, plugin-install, and `extractFields` format/delimiter gotchas) |
...
| File | What changed |
|---|---|
| [`2026-07-28-image-generation-session-notes.md`](migrations/2026-07-28-image-generation-session-notes.md) | **How image generation got onto the GPU fleet, and what it cost to learn** — written for the team as a narrative. The arc from a first-party Rust server to LocalAI (and why the original survey was wrong), then nine failures found on the live cluster with the lesson from each: a model card's precision is marketing (FP8/~8 GB vs **FP32/~33 GB**), seed memory is largest-shard × workers, a weights volume must fit the download **twice**, a PVC cannot shrink, readiness that gates on the wrong stage (twice, in opposite directions), `f16` is not a dtype knob, `PRELOAD_MODELS_CONFIG` ≠ `MODELS_CONFIG_FILE`, the gallery's tuning is for someone else's hardware, and a pricing basis that was wrong fleet-wide. Includes the mental model for how LocalAI, PVCs, backends and weights actually fit together. **Read before adding a model or pricing anything.** |
| [`2026-07-27-gpu-fleet-followups.md`](migrations/2026-07-27-gpu-fleet-followups.md) | **Open punch-list after the GPU-fleet build-out** (ADR-0094–0100): what still needs a human (the Discord webhook — secret FIRST, or the whole notification policy breaks; **building and pushing `z-image-turbo-server:v0.2.0`, which blocks the image-model deploy**), what we are currently guessing (no concurrency benchmarks, LMCache unproven and possibly harmful, the inherited 3.45× duty-cycle uplift, Z-Image-Turbo federated without a load gate at a placeholder price), and known limits (`/v1/models` listing internal-only models — RESOLVED via `AIGatewayRoute.spec.hostnames`, [#797](https://github.com/ADORSYS-GIS/ai-helm/pull/797); any `grafana.ini` change wipes all dashboards and needs a grafana-operator restart) |
...
| [`2026-07-31-dns01-route53-delegation-audit.md`](migrations/2026-07-31-dns01-route53-delegation-audit.md) | ✅ Resolved 2026-08-23 — `ai.camer.digital` is NS-delegated to Route53 while `cert-cloudflare` only serves `camer.digital`; fixed with a dedicated `cert-route53` ClusterIssuer (home-os#138/#139). `traefik/ai-certificate` turned out to be dead config and was deleted, not reissued (home-os#140/#141). Root cause, evidence, remediation options + resolution |
