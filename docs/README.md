...
| [`librechat-chain-of-agents.md`](./librechat-chain-of-agents.md) | **Chain-of-Agent use-case catalogue** (ticket #414, epic #409): five multi-agent patterns (sprint-to-GitHub ticket factory, doc-drift remediation, DoR ticket grooming, AI PR-review, external-team envoy) — each with its agent combination, inputs, human gate, findings + the cross-cutting findings (linear-chain limitation, adversarial verification, Projects v2 gap) and the most-promising-candidates ranking |
...
| [`ratelimit-quota-observability.md`](./ratelimit-quota-observability.md) | **Rate-limit quota — who's consuming and how much, from the limiter's LIVE Redis counters** (ADR-0070): the only place current-window budget state exists. The Redis key shape (`rule-2`=free/`rule-7`=pro budget, `<window>`=30-day bucket epoch); two read paths over the same keys — a `prometheus-redis-exporter` → Mimir leaderboard (`gateway_ratelimit_spend_micro_usd`, key parsed in ServiceMonitor metricRelabelings, `window` kept to avoid rollover collisions) + a direct `redis-datasource` tmscan census (the `tlsAuth:true`-enables-TLS plugin gotcha); raw consumption only; connectivity/secrets + the rule-index→plan coupling caveat |
| [`chat-observability.md`](./chat-observability.md) | **Phoenix-style chat content, from the AI Gateway's already-live OpenInference traces** (ADR-0077 + ADR-0079): the ext-proc has been sending full-content spans to Tempo all along (`llm.input_messages.*`/`output_messages.*`), true in Phoenix's era too; the `chat-overview` (no input, embeddings excluded, live trace feed — read content here) and `chats-by-user` (Keycloak-sourced `$user` picker, ADR-0063; per-request Loki metadata log) dashboards; why per-user CONTENT is a confirmed structural dead-end (ADR-0079: the AIEG ext-proc is Envoy filter #1, before Authorino, so `spanRequestHeaderAttributes` sees no `x-oidc-*` — proven via live config_dump; `filterOrder` reorder declined), the Postgres-variable `rawSql` gotcha, and why single-chat drill-down isn't built (no conversation id forwarded yet) |
...
| [`commit-conventions.md`](./commit-conventions.md) | **Commit message structure (ENFORCED)** — Conventional Commits: every type + its release-please/ADR-0082 version-bump effect, scope = chart dir (path-based attribution), breaking changes, why the PR title matters (squash), the AI co-author trailer. Enforcement = local `commit-msg` hook + the `Commit Lint` CI gate, both via the single validator `tools/commit-lint.sh`. |
