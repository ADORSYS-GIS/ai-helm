...
| [`chat-observability.md`](./chat-observability.md) | **Phoenix-style chat content, from the AI Gateway's already-live OpenInference traces** (ADR-0077): the ext-proc has been sending full-content spans to Tempo all along (`llm.input_messages.*`/`output_messages.*`); the `chat-overview` (no input, embeddings excluded) and `chats-by-user` (Keycloak-sourced `$user` picker, ADR-0063) dashboards, the `controller.spanRequestHeaderAttributes` fix pending in ai-helm-values to attribute spans by user, the SQL-injection-avoidance note, and why single-chat drill-down isn't built (no conversation id forwarded yet) |
