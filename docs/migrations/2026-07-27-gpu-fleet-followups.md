...
| The seed Job's 6 GiB memory limit is fleet-wide, not per-model | Untested against Z-Image-Turbo's ~10 GB safetensors shards (hf_xet is memory-hungry on large files; 2 GiB OOM-killed a seed pod historically). If it OOMs, the limit becomes a per-model knob |
