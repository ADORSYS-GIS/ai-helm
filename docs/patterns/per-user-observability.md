...
| Dashboard consumes the labels | `tools/dashboards/src/dashboards/envoy_ai_gateway/per_user.py` (generated → `charts/observability-dashboards/files/envoy-ai-gateway/per-user.json`) | Reads the **precomputed Mimir metrics** (ADR-0058) — the attribution labels ride along as metric labels, so the per-user cost/token/request/model/azp/latency panels and the email/azp/model variables query Mimir (instant, no log scan). Only the two status-code panels stay on Loki (`response_code` is deliberately not a metric label). |
