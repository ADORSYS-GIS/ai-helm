...
The LibreChat Coder Agent is a specialized leaf assistant (`model: adorsys-coder-pro`) responsible for autonomous developer workspace creation, in-workspace application scaffolding (Next.js + tRPC + Keycloak auth), and automated HTTP port sharing management.
