    ...
    return (
        ...
        .with_panel(
            _stat_panel(
                ...
                expr=f'{M_REQUEST_SUCCESS}{{finished_reason="error",model_name=~"$model_name"}}',
                unit="short",
                thresholds=_thresholds(("green", None), ("red", 1)),
                grid=(4, 4, 4, 0),
            )
        )
        .with_panel(
            _stat_panel(
                ...
                expr=f"rate({M_GENERATION_TOKENS}{_SEL}[5m])",
                unit="tokens/s",
                thresholds=_thresholds(("green", None)),
                grid=(4, 4, 8, 0),
            )
        ...
        .with_panel(
            _stat_panel(
                ...
                expr=f"rate({M_PROMPT_TOKENS}{_SEL}[5m])",
                unit="tokens/s",
                thresholds=_thresholds(("green", None)),
                grid=(4, 4, 12, 0),
            )
        ...
        .with_panel(
            _stat_panel(
                ...
                expr=f"rate({M_E2E_LATENCY_SUM}{_SEL}[5m]) / rate({M_E2E_LATENCY_COUNT}{_SEL}[5m])",
                unit="s",
                thresholds=_thresholds(("green", None), ("yellow", 1)),
                grid=(4, 4, 16, 0),
            )
        )
        .with_panel(
            _stat_panel(
                title="Avg TTFT",
                expr=f"rate({M_TTFT_SUM}{_SEL}[5m]) / rate({M_TTFT_COUNT}{_SEL}[5m])",
                unit="s",
        ...
        .with_panel(
            _timeseries_panel(
                ...
                unit="s",
                grid=(8, 12, 0, 5),
                targets=_percentile_targets(M_E2E_LATENCY_BUCKET),
            )
        )
        .with_panel(
            _timeseries_panel(
                ...
                unit="s",
                grid=(8, 12, 12, 5),
                targets=_percentile_targets(M_TTFT_BUCKET),
            )
        )
        .with_panel(
            _timeseries_panel(
                title="Per-Token Latency (Inter-Token)",
                unit="s",
                grid=(8, 12, 0, 13),
                targets=_percentile_targets(M_INTER_TOKEN_BUCKET),
            )
        )
        .with_panel(
            _timeseries_panel(
                title="Prompt Processing Latency",
                unit="s",
                grid=(8, 12, 12, 13),
                targets=_percentile_targets(M_PREFILL_BUCKET),
            )
        )
        ...
        .with_panel(
            _timeseries_panel(
                ...
                grid=(8, 12, 0, 22),
                targets=[
                    _prom_target(f"rate({M_GENERATION_TOKENS}{_SEL}[5m])", legend="{{model_name}}")
                ],
            )
        )
        .with_panel(
            _timeseries_panel(
                ...
                grid=(8, 12, 12, 22),
                targets=[
                    _prom_target(f"rate({M_PROMPT_TOKENS}{_SEL}[5m])", legend="{{model_name}}")
                ],
            )
        )
        .with_panel(
            _timeseries_panel(
                ...
                grid=(8, 12, 0, 30),
                targets=[
                    _prom_target(f"rate({M_REQUEST_SUCCESS}{_SEL}[5m])", legend="Success"),
                    _prom_target(
                        f'{M_REQUEST_SUCCESS}{{finished_reason="error",model_name=~"$model_name"}}',
                        legend="Failed",
                    ),
                ],
            )
        ...
        .with_panel(
            _timeseries_panel(
                ...
                unit="short",
                grid=(8, 12, 12, 30),
                # Two series, not three: the original third target plotted
                # M_NUM_WAITING a second time under the legend "Swapped". vLLM v1
                # has no swapped state, so that was the same line drawn twice with
                # a misleading name.
                targets=[
                    _prom_target(f"{M_NUM_RUNNING}{_SEL}", legend="Running"),
                    _prom_target(f"{M_NUM_WAITING}{_SEL}", legend="Waiting"),
                ],
            )
        )
        ...
        .with_panel(
            _timeseries_panel(
                ...
                unit="percent",
                grid=(8, 12, 0, 39),
                thresholds=_thresholds(("green", None), ("yellow", 70)),
                targets=[_prom_target(f"{M_KV_CACHE_USAGE}{_SEL}", legend="{{model_name}}")],
            )
        )
        .with_panel(
            _timeseries_panel(
                title="Prefix Cache Hits/s",
                unit="short",
                grid=(8, 12, 12, 39),
                targets=[
                    _prom_target(f"rate({M_PREFIX_CACHE_HITS}{_SEL}[5m])", legend="{{model_name}}")
                ],
            )
        )
        .with_panel(
            _timeseries_panel(
                ...
                unit="percent",
                grid=(8, 12, 0, 47),
                thresholds=_thresholds(("green", None), ("yellow", 50)),
                targets=[
                    _prom_target(
                        f"rate({M_PREFIX_CACHE_HITS}{_SEL}[5m]) / (rate({M_PREFIX_CACHE_HITS}{_SEL}[5m]) + rate({M_PREFIX_CACHE_QUERIES}{_SEL}[5m]))",
                        legend="{{model_name}}",
                    )
                ],
            )
        ...
        .with_panel(
            _timeseries_panel(
                title="KV Cache & Preemptions",
                unit="short",
                grid=(8, 12, 12, 47),
                targets=[
                    _prom_target(f"{M_KV_CACHE_USAGE}{_SEL}", legend="Usage %"),
                    _prom_target(f"{M_NUM_PREEMPTIONS}{_SEL}", legend="Preemptions"),
                ],
            )
        )
        .with_panel(
            _row("System Health (DCGM exporter not running - GPU metrics unavailable)", y=55)
        )
    )
