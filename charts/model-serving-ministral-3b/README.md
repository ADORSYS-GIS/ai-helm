...
- **⚠️ Tool-call parser is `mistral`** (not `hermes`) — Ministral uses Mistral's
  native `[TOOL_CALLS]` format. Note: the community AWQ repo's chat template is
  degraded (no tool-input formatting); vLLM's `--tool-call-parser=mistral` handles
  output extraction, but tool definitions may not be injected into the prompt
  unless a `--chat-template` override is supplied.
...
- `modelServing.controllers.main.containers.model.args` — vLLM flags include
  ...
  `--tool-call-parser=mistral`. ⚠️ model probes use bjw `custom: true` so they hit
