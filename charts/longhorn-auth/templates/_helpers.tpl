{{/*
longhorn-auth.argocd.destinationClusterRef
...
itself runs in. Mirror of homepage/lightbridge/coder/librechart/ai-models'
helper.

Input  : the `argocd.destination` dict.
Output : exactly one YAML line — `name: "…"` or `server: "…"`.
*/}}
{{- define "longhorn-auth.argocd.destinationClusterRef" -}}
{{- $d := . | default dict -}}
{{- $name := $d.name | default "" -}}
