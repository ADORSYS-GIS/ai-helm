...
- **⚠️ Port layout:** the model server binds HTTP `:8080` **and** KServe gRPC `:8081`;
  ...
  with "Failed to bind to address [::]:8081").
...
| 1 | `StatefulSet qwen3-4b` (containers `model` + `proxy`) + `Service qwen3-4b:8090` + `Ingress qwen3-4b` | **bjw** | the model + Caddy sidecar; the Service → the Ingress (className traefik, cert-manager annotation) |
