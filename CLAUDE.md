...
- **cert-manager** — controller (in `kube-system`, `--cluster-resource-namespace=kube-system`), CRDs, **and** the shared cluster-scoped ClusterIssuers (`cert-home-cert-http`, `self-signed-ca`, `cert-cloudflare`) are deployed by `home-os` (`charts/cert`, apps `cert` + `cert-remote`). home-os has **`config.enableGatewayAPI: true`** so cert-manager solves ACME HTTP-01 via `gatewayHTTPRoute` + honours the `cert-manager.io/cluster-issuer` gateway-shim. This repo only references issuers via annotations. ⚠️ The `cert-home-cert-envoy` issuer is **RETIRED** — `api.ai.camer.digital` TLS now comes from an **in-chart ns ACME `Issuer` + HTTP-01 `gatewayHTTPRoute`** solver (`charts/core-gateway`, `gateway.acmeHttp01.enabled`), no DNS token. `cert-cloudflare` (DNS-01, the only wildcard-capable issuer) is **live** — its `cloudflare-secret` in kube-system has been ESO-provisioned since 2026-07-22. ⚠️ **But it can only issue for names in zones CLOUDFLARE SERVES.** `camer.digital` is on Cloudflare; `ai.camer.digital`, `coder-ai.camer.digital` (AWS Route53) and `observe.camer.digital` (Google) are **NS-delegated away**. Point a DNS-01 `Certificate` at a delegated name and it fails *silently-ish*: cert-manager writes the `_acme-challenge` TXT into the Cloudflare zone, the `Challenge` reports `Presented: true`, and then stalls forever on `not yet propagated` because Route53 answers the lookup. Nine orphaned `_acme-challenge` TXT records (ai, coder-ai, coder-ai.ai, mlops, serverless.mlops) sit in the Cloudflare zone from repeat attempts. **Always `dig +short NS <parent-of-your-wildcard>` first** — non-Cloudflare NS ⇒ `cert-cloudflare` cannot issue for it. This is why Coder's workspace wildcard is `*.coder-ws.camer.digital` and NOT under `coder.ai.camer.digital` (`docs/integrations/coder-workspace-urls.md`). Escaping the trap needs a home-os change: a Route53 solver in `charts/cert` (+ an AWS IAM key with `route53:ChangeResourceRecordSets` — the ESO user `k8s-cluster-secret-manager` is **denied** Route53), or an `_acme-challenge` CNAME delegation into `camer.digital` plus `cnameStrategy: Follow`. HTTP-01 (`cert-home-cert-http`) is unaffected by delegation — it validates over the LB — but cannot do wildcards. Don't re-add a cert chart here. **Vanity-domain redirects (ADR-0053):** we own `kivoyo.com` (DNS already pointing at our LBs) and redirect it to the canonical `camer.digital` — `api.ai.kivoyo.com → api.ai.camer.digital` via `charts/core-gateway` `gateway.redirectHosts[]` (each entry → its own HTTPS listener + ACME cert reusing the in-chart issuer + a `RequestRedirect` HTTPRoute, `307`), and `ai.kivoyo.com → ai.camer.digital` via a Traefik `redirectRegex` Middleware + Ingress in `charts/librechat-app` `rawResources` (`302`). Temporary + path-preserving; add the next vanity host the same way (one `redirectHosts` entry).
