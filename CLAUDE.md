...
- **SonarQube is the ONE app that does NOT float from OCI — it's version-LOCKSTEP-pinned (ADR-0065).** Free **Community Build** can't do inline PR decoration, so we add the third-party `mc1arke/community-branch-plugin`, whose `major.minor` MUST match the Community Build version and which LAGS the monthly Community Build train. So SonarQube's image (`community.buildNumber`) ↔ the `plugins.install` jar ↔ the two `-javaagent` `env` filenames are bumped **together** (today `26.5.0.122743` ↔ `26.5.0`), and it's **excluded from argocd-image-updater** (auto-bump ahead of the plugin = web/CE JVM won't start). The chart default image is the PAID Server line — `community.enabled: true` selects the free `…-community` build. App-of-Apps orchestrator (`charts/sonarqube{,-db,-secrets}`, controlPlane) mirroring lightbridge; CNPG `sonarqube-db`; UI SSO = native **SAML** to Keycloak (client `sonarqube` in `charts/keycloak-baseline` — the realm-configmap template now supports `protocol: saml` + per-client `attributes`/`protocolMappers`). ⚠️ The new `sonarqube` namespace has NO hetzner deny-baseline; the deps-overlay CiliumNetworkPolicies (in `ai-helm-values`) make it self-contained default-deny (allow DNS + `api.github.com` + DB + S3). ⚠️ `vm.max_map_count=524288` is a hetzner-k8s NODE sysctl prereq (the chart's privileged `initSysctl` is disabled for k3s `baseline` PSS). The GitHub App (PR decoration) is bound at RUNTIME via the SonarQube UI/`alm_settings` API, not a chart value. See `docs/sonarqube.md`.
