...
Deploys are pinned to an **immutable release tag** — currently **`release-2026.06.10-v03`** (cut over from the branch `claude/magical-bohr-390242` on 2026-06-08). `main` is **never** a deploy target. Every self-referencing `targetRevision` pins the tag: `argocd.selfTargetRevision` + the per-app self-Source revisions in `charts/apps/values.yaml`, and the orchestrator children in `charts/ai-models/values.yaml`, `charts/librechart/values.yaml`, and `charts/observability/values.yaml`. The canonical note lives at `argocd.selfTargetRevision` in `charts/apps/values.yaml`. (`HEAD` revisions that point at *other* repos are unaffected.)
...
- **⚠️ `/mcp/*` is the one carve-out from Authorino (ADR-0038).** Each MCPRoute carries `securityPolicy.oauth` (leaf `charts/mcp`, shared config in `charts/mcps` `oauth:`), and an EG route-level SecurityPolicy **displaces** the gateway-attached Authorino policy whole-policy (no merge) — so on MCP routes JWT verification is Envoy's native `jwt_authn` filter (same Keycloak issuer), the gateway itself serves the MCP-spec/RFC 9728 discovery surface unauthenticated (`/.well-known/oauth-protected-resource/mcp/<name>` path-insertion form + AS-metadata aliases + 401 `WWW-Authenticate resource_metadata` challenge; plus our non-spec path-appended alias `<path>/.well-known/oauth-protected-resource` via HTTPRoute+DirectResponse+allow-all SecurityPolicy), and the ADR-0011 `x-oidc-*` set is re-stamped via `claimToHeaders` (caveat: object/array claims arrive base64url(JSON), not plain JSON). Rate-limit descriptors are NOT stamped on `/mcp/*` (no MCP rate limiting). The `mcps` orchestrator's `targetRevision` is bumped by `tools/release.sh` via literal old→new swap — if it ever drifts from the apps chart's tag (a 2026-06-09 merge did this), the swap silently skips it; keep it in lockstep.
  - **⚠️ External HTTPS MCP backends need a TLS-socket patch (ADR-0039); context7 is self-hosted.** AIEG stamps a placeholder **`dummy.transport_socket`** (empty `UpstreamTlsContext`, no SNI) on the cluster it generates for each external MCP backend, and EG's `BackendTLSPolicy` / inline `Backend.spec.tls.sni` **never reach it** (verified: inline tls.sni had zero effect) → upstream TLS to CDN-fronted MCPs is sent with empty SNI → handshake fails → "failed to create MCP session to any backend". (Self-hosted plain-HTTP MCPs — brave, terraform — have no TLS hop, unaffected.) Fix: an **`EnvoyPatchPolicy`** in `charts/core-gateway` (`mcpBackendTls.backends`, namespace `converse-gateway` = the Gateway's ns) replaces the dummy with a real `envoy.transport_sockets.tls` (SNI + system-CA `/etc/ssl/certs/ca-certificates.crt`). ⚠️ **Brittle**: it targets the AIEG-derived cluster name `httproute/<mcpNamespace>/ai-eg-mcp-br-<name>-<name>/rule/0` — re-verify after any AIEG bump. ⚠️ **RSA upstreams only** (firecrawl, refero work); **context7 serves an ECDSA cert that Envoy's BoringSSL rejects** (`BAD_ECC_CERT`) SNI-or-not → context7 was **DROPPED** (2026-06-11): self-hosting needs a custom image (no published context7 HTTP image) + Upstash Redis (v3 HTTP session store), both against repo constraints. Active MCPs: brave, terraform (self-hosted), firecrawl, refero (external). NB the **AIEG v0.6.0→v0.7.0** bump (`charts/apps` `aieg`+`aieg-crd`, lockstep; EG floor 1.7.0 / we run v1.8.0; `securityPolicy.oauth` unchanged; AIGatewayRoute 128→15 rule cap N/A) was a red herring for this (mcpproxy unchanged) but is the right latest baseline. Full diagnosis + repro recipe (config_dump / stats / openssl): `docs/2026-06-10-mcp-external-server-proxy-debug.md`.
