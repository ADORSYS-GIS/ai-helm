...
- **Grafana AI assistant (`grafana-llm-app`) routes through the INTERNAL gateway plane, not the public host (ADR-0062).** The plugin sends a *static* `Authorization: Bearer <key>`, which only the internal plane accepts (the external `api.ai.camer.digital` needs a short-lived Keycloak JWT). Config is in `ai-helm-values` `environments/prod/values/grafana.yaml` (plugin install + an `extraObjects` provisioning ConfigMap → `openAI.url: https://core-gateway-internal.envoy-gateway-system.svc/v1`, model map `base→gemma-4`/`large→glm-5`) + the `deps/grafana` overlay (gateway-key ExternalSecret, a `self-signed-ca` CA leaf mounted into `/etc/ssl/certs` so Go trusts the internal listener while keeping public roots, Cilium egress to `envoy-gateway-system:443`) + a dedicated `internal-key-grafana` apiKey in `deps/security-policies` (own `x-account-id` cost bucket). ⚠️ **Out-of-band, values-repo-first:** the ssegning-aws property `grafana_llm_gateway_key` must exist before sync (else both ExternalSecrets sit in `SecretSyncedError`), and the key env is `optional: true`, so after the key first lands you must **re-roll the Grafana pod** (env binds at pod start). mcp-grafana is a separate follow-up.
